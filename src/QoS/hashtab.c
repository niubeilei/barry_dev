                               ...
                               u32 size)
{
	...
	p = (struct hashtab *)aos_malloc_atomic(sizeof(*p));
	if (p == NULL)
		return p;
	...
	p->keycmp = keycmp;
	p->htable = (struct hashtab_node **)aos_malloc_atomic(sizeof(*(p->htable)) * size);
	if (p->htable == NULL) {
		aos_free(p);
		return NULL;
	}
...
int hashtab_insert(struct hashtab *h, void *key, void *datum)
{
	...
	if (!h || h->nel == HASHTAB_MAX_NODES)
		return -EINVAL;
	//printk(KERN_ERR "hashtable length: %d", h->nel);
	...
	newnode = (struct hashtab_node *)aos_malloc_atomic(sizeof(*newnode));
	if (newnode == NULL)
		return -ENOMEM;
