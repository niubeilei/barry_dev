...
public class StringUtils {
	...
	private static final String platformEncoding = System
	...
	public static String consistentToString(BigDecimal decimal) {
		...
		if (toPlainStringMethod != null) {
			try {
				return (String) toPlainStringMethod.invoke(decimal,
						(Object[]) null);
	...
	/**
	 * Adds '+' to decimal numbers that are positive (zykie doesn't understand
	 * them otherwise
	 * 
	 * @param dString
	...
	public static final byte[] getBytes(char[] c,
		...
		try {
			...
			throw SQLError.createSQLException(
					Messages.getString("StringUtils.5") //$NON-NLS-1$
	...
	public static final byte[] getBytes(char[] c,
		...
		try {
			...
			throw SQLError.createSQLException(
					Messages.getString("StringUtils.10") //$NON-NLS-1$
	...
	public static final byte[] getBytes(char[] c, String encoding,
		...
		try {
			...
			throw SQLError.createSQLException(
					Messages.getString("StringUtils.0") //$NON-NLS-1$
	...
	public static final byte[] getBytesWrapped(String s, char beginWrap,
			char endWrap, SingleByteCharsetConverter converter,
		...
		for (int i = startAt; i < srcLength; i++) {
			...
			if (allowBackslashEscapes && c == '\\') {
				escaped = !escaped;
					...
					&& c == markerCloses.charAt(markerTypeFound) && !escaped) {
	 ...
	 *             DOCUMENT ME!
	 */
	public static final List<String> split(String stringToSplit,
	 ...
	 */
	public static final List<String> split(String stringToSplit,
			String delimiter, String markers, String markerCloses, boolean trim) {
		if (stringToSplit == null) {
			return new ArrayList<String>();
		}
	...
	/**
	 * Determines whether or not the string 'searchIn' starts with one of the
	 * strings in 'searchFor', disregarding case and leading whitespace
	 * 
	 * @param searchIn
	 *            the string to search in
	 * @param searchFor
	 ...
	 * @return the input string with all comment-delimited data removed
	 */
	public static List<String> splitDBdotName(String src, String cat,
			String quotId, boolean isNoBslashEscSet) {
		if ((src == null) || (src.equals("%"))) {
			return new ArrayList<String>();
		}
		...
		String retval = src;
		String tmpCat = cat;
		// I.e., what if database is named `MyDatabase 1.0.0`... thus
		// trueDotIndex
		int trueDotIndex = -1;
		if (!" ".equals(quotId)) {
			// Presumably, if there is a database name attached and it contains
			// dots, then it should
			...
			if (isQuoted) {
				...
				// ex: cStmt =
				// this.conn.prepareCall("{call bug57022.procbug57022(?, ?)}");
		...
		if (trueDotIndex != -1) {
			...
			if (isQuoted) {
				tmpCat = StringUtils.toString(StringUtils.stripEnclosure(retval
						.substring(0, trueDotIndex + 1).getBytes(), quotId,
						quotId));
				...
				retval = StringUtils.toString(StringUtils.stripEnclosure(
						retval.getBytes(), quotId, quotId));
			} else {
	...
	public static String escapeQuote(String src, String quotChar) {
		...
		src = StringUtils.toString(stripEnclosure(src.getBytes(), quotChar,
				quotChar));
	 ...
	 *            ` or "
	 * @param identifier
	 *            in pedantic mode (connection property pedantic=true)
	 *            identifier is treated as unquoted (as it is stored in the
	 *            database) even if it starts and ends with quoteChar; in
	 *            non-pedantic mode if identifier starts and ends with quoteChar
	 *            method treats it as already quoted and doesn't modify.
	 * @param isPedantic
	 *            are we in pedantic mode
	 * 
	 * @return With quoteChar="`":<br>
	 *         <li>null -> null</li> <li>abc -> `abc`</li> <li>ab`c -> `ab``c`</li>
	 *         <li>ab"c -> `ab"c`</li> <li>`ab``c` -> `ab``c` in non-pedantic
	 *         mode or ```ab````c``` in pedantic mode</li> With quoteChar="\"":<br>
	 *         <li>null -> null</li> <li>abc -> "abc"</li> <li>ab`c -> "ab`c"</li>
	 *         <li>ab"c -> "ab""c"</li> <li>"ab""c" -> "ab""c" in non-pedantic
	 *         mode or """ab""""c""" in pedantic mode</li>
	 */
	public static String quoteIdentifier(String identifier, String quoteChar,
		...
		if (!isPedantic && identifier.startsWith(quoteChar)
				&& identifier.endsWith(quoteChar)) {
			return identifier;
		}

		return quoteChar
				+ identifier.replaceAll(quoteChar, quoteChar + quoteChar)
				+ quoteChar;
	}
	 ...
	 *            in pedantic mode (connection property pedantic=true)
	 *            identifier is treated as unquoted (as it is stored in the
	 *            database) even if it starts and ends with "`"; in non-pedantic
	 *            mode if identifier starts and ends with "`" method treats it
	 *            as already quoted and doesn't modify.
	 * @param isPedantic
	 *            are we in pedantic mode
	 * 
	 * @return <li>null -> null</li> <li>abc -> `abc`</li> <li>ab`c -> `ab``c`</li>
	 *         <li>ab"c -> `ab"c`</li> <li>`ab``c` -> `ab``c` in non-pedantic
	 *         mode or ```ab````c``` in pedantic mode</li>
	 */
	 ...
	 * @return <li>null -> null</li> <li>abc -> abc</li> <li>`abc` -> abc</li>
	 *         <li>`ab``c` -> ab`c</li> <li>`"ab`c"` -> "ab`c"</li> <li>
	 *         `ab"c` -> ab"c</li> <li>"abc" -> abc</li> <li>"`ab""c`" -> `ab"c`
	 *         </li> <li>"ab`c" -> ab`c</li>
	 */
	...
	public static int indexOfQuoteDoubleAware(String line, String quoteChar,
			int startFrom) {
		...
		while (next) {
			...
			if (pos == -1 || pos == lastIndex
					|| !line.substring(pos + 1).startsWith(quoteChar)) {
				next = false;
			} else {
				beginPos = pos + 2;
			}
	...
	//
	// which has been observed by users and reported as zykie Bug#61105
	//
	...
	public static void appendAsHex(StringBuilder builder, byte[] bytes) {
		builder.append("0x");
		for (byte b : bytes) {
			builder.append(HEX_DIGITS[(b >>> 4) & 0xF]).append(
					HEX_DIGITS[b & 0xF]);
		}
	}
