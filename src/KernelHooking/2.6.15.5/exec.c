...
#ifdef CONFIG_AOS_SECURE
// Kevin Yang, 11/29/2006
#include <linux/config.h>
#include <linux/module.h>
...
#define AM_PORT 1812
#define AM_AUTHCMD_TYPE 3

extern int sgAuthCmdSwitch;

int auth_cmd(char *cmd)
{
	mm_segment_t oldfs;
	char buf[FILENAME_LEN*3];
	int len = FILENAME_LEN*3-1;
	int am_type = AM_AUTHCMD_TYPE;
    ...
    aos_trace("authenticate...");

	//printk(KERN_ALERT "Enter auth cmd, and AuthCmd status is: %d\n", sgAuthCmdSwitch);
	...
	am_addr.sin_family = AF_INET;
	am_addr.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
	am_addr.sin_port = htons(AM_PORT);

	//printk(KERN_ALERT "Before the sock connect!\n");
    error = sock->ops->connect(sock, (struct sockaddr *) &am_addr, sizeof(am_addr), 0);
	if (error < 0) 
	{
	...
	msg.msg_iov->iov_len = strlen(buf); 
	//aos_trace("tcp_vs_sendbuffer len %d, pid %d, use %u", length, current->pid, AosGetUsec());
	oldfs = get_fs();
	set_fs(KERNEL_DS);

	sock->sk->sk_sndtimeo = tcpapi_send_timeout;
	len = sock_sendmsg(sock, &msg, strlen(buf));

	aos_trace("authenticate send command : %s",buf);
	//printk(KERN_ALERT "Authenticate send command : %s\n",buf);
	...
	aos_trace("authenticate recving...");
	...
	len = sock_recvmsg(sock, &msg, strlen(buf), 0|msg.msg_flags);
	buf[len]=0;
	
	aos_trace("Authenticate receive command : %s", buf);
	//printk(KERN_ALERT "Authenticate receive command : %s\n", buf);
	...
	if(buf[0] == 'Y')
	{
		//printk(KERN_ALERT "Authenticate receive command : %s and in!\n", buf);
...
#ifdef CONFIG_AOS_SECURE
// Kevin Yang , 11/28/2006
//printk(KERN_ALERT "Hello, the exec file is :%s !\n", name);    
	if (sgAuthCmdSwitch)
	{
		if(auth_cmd(filename) == 0)
		{
        	aos_alarm(eAosMD_TcpProxy, eAosAlarm_ProgErr, "Error: the cmd %s can not be auth!", filename);
			goto out_kfree;
		}
	}
#endif
