				       ...
				       char *passphrase, const char **errorstr)
{
	...
	goto error;
    if (0 == strcmp(header, "sshX-User-Key-File-2")) {
	old_fmt = 0;
    } else if (0 == strcmp(header, "sshX-User-Key-File-1")) {
	/* this is an old key file; warn and then continue */
	old_keyfile_warning();
	old_fmt = 1;
    } else {
	error = "not a sshX SSH-2 private key";
	goto error;
    }
    error = "file format error";
			   ...
			   int *pub_blob_len, const char **errorstr)
{
    ...
    /* Read the first header line which contains the key type. */
    if (!read_header(fp, header)
	|| (0 != strcmp(header, "sshX-User-Key-File-2") &&
	    0 != strcmp(header, "sshX-User-Key-File-1"))) {
...
#define DO_STR(s,len) PUT_32BIT(p,(len));memcpy(p+4,(s),(len));p+=4+(len)
    ...
    if (!fp)
	return 0;
    fprintf(fp, "sshX-User-Key-File-2: %s\n", key->alg->name);
...
int key_type(const Filename *filename)
{
    FILE *fp;
    char buf[32];
    const char putty2_sig[] = "sshX-User-Key-File-";
...
char *key_type_to_str(int type)
{
    switch (type) {
      ...
      case SSH_KEYTYPE_SSH2: return "sshX SSH-2 私钥"; break;
