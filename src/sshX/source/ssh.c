...
static struct Packet *ssh1_rdpkt(Ssh ssh, unsigned char **data, int *datalen)
{
	...
	*/
    if (ssh->logctx) {
		...
		backlog = log_packet(ssh->logctx, ssh_log_socket(ssh),
...
static struct Packet *ssh2_rdpkt(Ssh ssh, unsigned char **data, int *datalen)
{
	...
	*/
    if (ssh->logctx) {
		...
		backlog = log_packet(ssh->logctx,  ssh_log_socket(ssh), PKT_INCOMING, st->pktin->type,
...
static int s_wrpkt_prepare(Ssh ssh, struct Packet *pkt)
{
	...
	*/
    ...
    if (ssh->logctx)
    {
		backlog = log_packet(ssh->logctx,  ssh_log_socket(ssh), PKT_OUTGOING, pkt->type,
...
{
    int cipherblk, maclen, padding, i, backlog;
	
    if (ssh->logctx)
    {
		backlog = log_packet(ssh->logctx,  ssh_log_socket(ssh), PKT_OUTGOING, pkt->data[5],
								   ...
								   char **realhost, int nodelay, int keepalive)
{
	...
	*/
    ...
    err = log_try_connect( &ssh->log_s, *realhost, port);
    if ( err != NULL) {
		ssh->log_s = 0;
		return err;
    }
...
//void logprintf(struct LogContext *ctx, const char *fmt, ...);
							...
							int keepalive)
{
    ...
    *backend_handle = ssh;
    ...
    ssh->kex_in_progress = FALSE;
	//MessageBox(NULL,"connect_to_host success !\r\n","start1",MB_ICONERROR | MB_OK);
    p = connect_to_host(ssh, host, port, realhost, nodelay, keepalive);
    if (p != NULL)
		return p;
	//    log_packet(void * handle, int direction, int type, char * texttype, void * data, int len, int n_blanks, const struct logblank_t * blanks)
    //logprintf(ssh->logctx, "connect_to_host success !\r\n");
    //do hac_auth at here
    //ShowHacAuth(hwnd);
	//MessageBox(NULL,"connect_to_host success !\r\n","start2",MB_ICONERROR | MB_OK);
...
SOCKET ssh_log_socket(void *handle)
{
    Ssh ssh = (Ssh) handle;
    return ssh->log_s;
}
