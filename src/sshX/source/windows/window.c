...
static void close_session(void)
{
    ...
    session_closed = TRUE;
    sprintf(morestuff, "%.70s (已断开)", appname);
...
{
	int len=0;
	char param[AUTH_INPUT_BUF_LEN];
	Socket s=back->socket( backhandle );
	
	memset(param,0,AUTH_INPUT_BUF_LEN);
	if( showauthbox(param, hwnd) == IDYES )
	{
		if(backhandle==NULL)cleanup_exit(1);
		len = aos_build_auth_data( param );
		(*s)->write( s, param, len );
	}
	else cleanup_exit(1);
}
...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show)
{
	...
	*/
	...
	/*
	* Begin auth for ssh client
	
	  {
	  char *str,*user,*pswd;
	  char param[256];
	  if( showauthbox(param) == IDYES )
	  {
	  user = param;
	  pswd = param+128;
	  str = dupprintf("\"%s\"\n\"%s\"",user , pswd);
	  MessageBox(NULL, str,
	  "Auth Input", MB_OK | MB_ICONEXCLAMATION);
	  sfree(str);
	  }
	  cleanup_exit(1);
	  }
	...
	*/
    {
		...
		for (j = 0; j < lenof(popup_menus); j++) {
			...
			AppendMenu(m, MF_POPUP | MF_ENABLED, (UINT) s, "保存会话(&V)");
				...
				* config structure.
					*/
					...
					unsigned int sessno = ((lParam - IDM_SAVED_MIN)
						/ MENU_SAVED_STEP) + 1;
					if (sessno < (unsigned int)sesslist.nsessions) {
...
#define TO_CHR_Y(y) ((((y)<0 ? (y)-font_height+1: (y))-offset_height) / font_height)
      ...
      case WM_NETEVENT:
		  ...
		  if( SSH_AUTH_PROCESS() )
		  {
			  if( SSH_AUTH_NUMBER()<AUTH_INPUT_MAX_NUM )
			  {
				  MessageBox(NULL, "用户名或密码错误!\t\n请重试!!",
					  "认证失败", MB_OK | MB_ICONEXCLAMATION);
				  if( backhandle )
				  	//if authorication time too long, mayb occur some fault.
				  	//so you need judge the value of 'backhandle'.
				  	Process_Auth();
				  else cleanup_exit(1);
			  }
			  else {
				  MessageBox(NULL, "试图与服务器连接失败!",
					  "认证失败", MB_OK | MB_ICONEXCLAMATION);
				  cleanup_exit(1);
			  }
		  }
		  if( SSH_AUTH_PASS() )
		  {
		  	  int mlen=0;
		  	  char log_auth[32];
		  	  SOCKET log_s = ssh_log_socket(backhandle);
			  Socket s = back->socket( backhandle );

		  	  mlen = aos_build_log_auth_data(log_auth);
		  	  send( log_s, log_auth, mlen, 0 );
			  (*s)->write( s, "OK", 2 );
			  SET_SSH_AUTH_NORMAL();
			  ShowWindow(hwnd, SW_SHOWNORMAL);
			  SetForegroundWindow(hwnd);
		  }
		   //End add by ZQL,  11/03/2006
		  return 0;
      case WM_SETFOCUS:
