...
/*
 * Just for connect to log socket.
 */

void log_close(void *log_s)
{
	if(*((SOCKET *)log_s)){
		p_closesocket( *((SOCKET *)log_s) );
		*((SOCKET *)log_s)=0;
	}
}

char *log_try_connect(void *log_s, char *remote, short port)
{
    SOCKET s;
    SOCKADDR_IN a;
    DWORD err;
    ...
    *((SOCKET *)log_s) = s;
    ...
    if (s == INVALID_SOCKET) {
		err = p_WSAGetLastError();
		error_str = winsock_error_string(err);
		goto ret;
    }
	...
	*/
    {
		...
		a.sin_addr.s_addr = p_inet_addr(remote);
	...
	* plug.
	...
	case FD_CONNECT:
		...
		*/
		...
		//MessageBox(NULL,"FD_CONNECT","select_result", MB_OK );
		break;
	case FD_READ: // 1
		...
		*/
		...
		{
			//char tmp[32];
			//sprintf(tmp,"len = %d \n ssh_auth_status = %d.",ret,ssh_auth_status);
			//MessageBox(NULL,tmp,"FD_READ", MB_OK | MB_ICONEXCLAMATION);
		}
		if (ret < 0) {
			...
			{
				//char tmp[32];
				//sprintf(tmp,"len = %d .",ret);
				//MessageBox(NULL,tmp,"FD_READ", MB_OK | MB_ICONEXCLAMATION);
				aos_verify_auth( buf,  ret );
				//return 0;
			}
...
SOCKET get_log_socket(Socket s)
{
