...
char *auth_input = NULL;		/* char point to save hac auth user and passwsd */
...
static int CALLBACK AuthProc(HWND hwnd, UINT msg,
							 WPARAM wParam, LPARAM lParam)
{
    switch (msg) {
	case WM_INITDIALOG:
		...
		*/
		...
		SetFocus( hwnd );
		SetFocus( GetDlgItem( hwnd, IDC_EDITUSER ) );
		return 1;
	case WM_COMMAND:
		switch (LOWORD(wParam)) {
		case IDOK:
			if( auth_input != NULL )
			{
				GetDlgItemText( hwnd, IDC_EDITUSER, auth_input, AUTH_INPUT_BUF_LEN/2 );
				if( !TrimString(auth_input) )
				{
					MessageBox(hwnd, "用户名不能为空!",
						"输入错误", MB_OK | MB_ICONWARNING);
					SetDlgItemText( hwnd, IDC_EDITUSER, "" );
					break;
				}
				GetDlgItemText( hwnd, IDC_EDITPSWD, auth_input+AUTH_INPUT_BUF_LEN/2, AUTH_INPUT_BUF_LEN/2 );
			}
			SaneEndDialog( hwnd, 0 );
			return IDOK;
		case IDCANCEL:
		case WM_CLOSE:
			if( MessageBox(hwnd, "你确定要退出认证吗?",
						"sshX退出确认", MB_YESNO | MB_ICONQUESTION)==IDNO)break;
			SaneEndDialog( hwnd, 1 );
			return IDCANCEL;
		}
    }
    return 0;
						 ...
						 DLGPROC lpDialogFunc)
{
    ...
    wc.lpszClassName = "SSHXDialogBox";
    RegisterClass(&wc);
...
int showauthbox(void *input, HWND hwndparent )
{
	int ret=0;
	auth_input=(char *)input;

	// if press OK     button then return 0;
	// if press Cancel button then return 1.
    ret = SaneDialogBox(hinst,MAKEINTRESOURCE(IDD_AUTHBOX),hwndparent,AuthProc);

	return (ret?IDNO:IDYES);
}
