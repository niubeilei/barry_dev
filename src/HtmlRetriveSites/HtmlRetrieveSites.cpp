...
///////////////////////////////////////////////////////////////////////////////////
#include "HtmlRetriveSites/HtmlRetrieveSites.h"
	...
	OmnString &errmsg)
{
	...
	{
		mPageNumber = i+1;
		AosXmlTagPtr response = AosSengAdmin::getSelf()->retrieveDoc(siteid, mVpdArray[i], sessionId, isInEditor, cookies);
		if (!response)
			continue;
		...
		const OmnString &superVpdname)
{
	...
	else if (vpdtype == "PWT" || vpdtype == "comp_pane")
	{
	...
	const OmnString &superVpdname)
{
	...
	while (pane)
	{
		...
		if(pane_type == "PWT" || pane_type == "comp_pane")
		{
	...
	const OmnString &superVpdname)
{
	...
	else if (gic_type == "gic_tablist" || gic_type == "gic_contactlist")
	{
		// gic_vpdnames
		// need to achieve
	}else if (gic_type == "gic_breadcrumbs" || gic_type == "gic_html_menu")
	{
		...
		OmnString cont = gic->getAttrStr("gic_data");
		if (cont != "")
		{
		...
		AosXmlParser parser;
		AosXmlTagPtr root = parser.parse(cont, "");
		...
		while (record)
		{
			AosXmlTagPtr actions = gic_breadcrumbs->getFirstChild("actions");
			if (!actions)
			{
			...
			while (action)
			{
				OmnString actionid = action->getAttrStr("id", "");
		...
		{
			OmnString actionid = action_br->getAttrStr("id", "");
			if (actionid == "act_vpd" || actionid == "act_open_dialog")
			{
	...
	const OmnString &superVpdname)
{
	...
	while (parm)
	{
		if(parm->getAttrStr(AOSTAG_PNAME) == "vpdname" &&
		...
		OmnString &errmsg)
{
	...
	for (int i=0; i<ss; i++)
	{
		...
		OmnString parts[5];
		bool finished;
		int nn = split.splitStr(mVpdnamelist[i], ",", parts, 5, finished);
		if (nn != 5)
		{
