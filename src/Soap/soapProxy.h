	...
	{ soap = soap_new(); endpoint = "http://localhost:80"; if (soap && !soap->namespaces) { static const struct Namespace namespaces[] = 
{
	...
	/// Invoke 'procMsg' of service 'Service' and return error code (or SOAP_OK)
	virtual int ns__procMsg(char *appid, char *input, char **output) { return soap ? soap_call_ns__procMsg(soap, endpoint, NULL, appid, input, output) : SOAP_EOM; };
};
#endif
