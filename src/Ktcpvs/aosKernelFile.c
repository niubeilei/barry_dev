...
//
// Change History
// 07/28/2006 aosKernelFile.c created
// 08/02/2006 is changed :
//	 1) modfiy aos_fopen function to use "rwa" char mode
//	 2) add aos_fscanf function
///////////////////////////////////////////////////////////////////////
...
//KFILE *aos_fileopen(const char *filename, int flags, int mode) 
//{ 
...
KFILE *aos_fopen( const char *filename, const char *mode ) 
{ 
	int i=0,flags=0;
	...
	KFILE *filp = filp_open( filename, flags, S_IRUSR | S_IWUSR ); 
...
int aos_fseek(KFILE *filp, int offset, int whence) 
{ 
	int pos = filp->f_pos; 
...
int aos_fread(char *buf, int len, KFILE *filp) 
{ 
	int readlen; 
	mm_segment_t savefs; 
...
int aos_fgetc(KFILE *filp) 

{ 
	int len; 
	unsigned char buf[4]; 
	
	len = aos_fread((char *) buf, 1, filp); 
...
char *aos_fgets(char *str, int size, KFILE *filp) 
{ 
	char *cp; 
	int len, readlen; 
	mm_segment_t savefs; 
...
int aos_fwrite(char *buf, int len, KFILE *filp) 
{ 
	int writelen; 
	mm_segment_t savefs; 
	
	if (filp == NULL) {
		printk(  " %s : In function \'aos_fwrite\' , the filp is NULL !!\n", aos_LogChgTime2Str(get_seconds()) );
		return -ENOENT;
	}	
...
int aos_fputc(int ch, KFILE *filp) 
{ 
	int len; 
	unsigned char buf[4]; 
	
	buf[0] = (unsigned char) ch; 
	len = aos_fwrite(buf, 1, filp); 
...
int aos_fputs(char *str, KFILE *filp) 
{ 
	return aos_fwrite(str, strlen(str), filp); 
} 
...
int aos_fprintf(KFILE *filp, const char *fmt, ...) 
{ 
...
void aos_fscanf(KFILE *filp, const char *fmt, ...) 
{ 
	static char s_buf[LINEMAXLENTH]; 
	
	if( aos_fgets( s_buf, LINEMAXLENTH, filp ) == NULL )return;
	va_list args; 
...
//#define  AOS_LOG_FILENAME	"/var/log/aosMessager"

int aos_file_printk(const char *logfile, const char *fmt, ...) 
{ 
	KFILE *aos_file=NULL;
	static char s_buf[LINEMAXLENTH]; 
	va_list args; 
	
	if (( aos_file = aos_fopen( logfile, "a" )) == NULL ){
